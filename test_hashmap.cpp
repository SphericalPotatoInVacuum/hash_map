...
void const_check() {
  ...
  if (map.empty()) fail("incorrect empty method");

  static_assert(std::is_same<HashMap<int, int>::const_iterator,
                             decltype(map.begin())>::value,
                "'begin' returns not a const iterator");
  auto hash_f = map.hash_function();
  std::cerr << hash_f(0) << "\n";
  for (auto cur : map) std::cerr << cur.first << " " << cur.second << "\n";

  HashMap<int, int>::const_iterator it = map.find(3);
  if (it->second != 4) fail("not found 3, incorrect find or insert");
  it = map.find(7);
  if (it != map.end()) fail("found 7? incorrect find or insert");
...
void reference_check() {
  ...
  map[3] = 7;
  if (map[3] != 7 || map[0] != 0) fail("incorrect [ ]");
  auto it = map.find(4);
  if (it == map.end()) fail("not found 4, incorrect find or insert");
  it->second = 3;
  auto cur = map.find(4);
  if (cur->second != 3) fail("can't modificate through iterator");
  std::cerr << "ok!\n";
}
...
void hash_check() {
  ...
  if (second_hash_fn(17239) != 0) fail("wrong hash function in class");
  ...
  for (int i = 0; i < 1000; ++i) {
    stupid_map[i] = i + 1;
    if (stupid_hash_fn(i)) fail("wrong hash function in class");
  }
  if (stupid_map.size() != 1000) fail("Wrong size");
  std::cerr << "ok!\n";
}
...
void check_copy() {
  ...
  third[0] = 5;
  if (third.size() != 2) fail("Wrong size");
  first = third;
  second = second = first;
  if (first.find(0)->second != 5) fail("wrong find");
  if (second[0] != 5) fail("wrong [ ]");
  std::cerr << "ok!\n";
}
...
void check_iterators() {
  std::cerr << "check iterators...\n";
  {
    ...
    if (it++ != first.begin()) fail("bad post ++");
    if (!(it == first.end())) fail("bad post ++");
    if (++first.begin() != first.end()) fail("bad pre ++");
    first.erase(0);
    if (first.begin() != first.end()) fail("bad begin or end");
